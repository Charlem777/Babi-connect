...
class Partenaire(Base):
    ...
    abonnements = relationship("Abonnement", back_populates="partenaire")

    class Abonnement(Base):
        __tablename__ = "abonnements"
        id = Column(Integer, primary_key=True, index=True)
        partenaire_id = Column(Integer, ForeignKey("partenaires.id"))
        type_abonnement = Column(String(100))  # Ex: "Mensuel", "Annuel", "Premium", etc.
        date_debut = Column(DateTime, default=datetime.utcnow)
        date_fin = Column(DateTime)
        statut = Column(String(50))  # Ex: "actif", "expiré", "suspendu"
        partenaire = relationship("Partenaire", back_populates="abonnements")
