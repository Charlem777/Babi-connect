<template>
  <div class="relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group h-96 transform hover:-translate-y-2">
    ...
    <div class="relative h-48 overflow-hidden">
      ...
      />
      <!-- Gradient overlay pour événement -->
      <div class="absolute inset-0 bg-gradient-to-t from-purple-900/70 via-transparent to-transparent"></div>
      
      <!-- Badge type événement -->
      <div class="absolute top-3 left-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
        🎉 Événement
      </div>
      
      <!-- Date si disponible -->
      <div v-if="offre.expire_le" class="absolute top-3 right-3 bg-black/50 backdrop-blur-sm text-white px-2 py-1 rounded-lg text-xs">
        {{ formatEventDate(offre.expire_le) }}
      </div>
    ...
    <div class="p-4 h-48 flex flex-col">
      ...
      <div class="mb-3">
        <h3 class="font-bold text-gray-900 text-lg mb-1 line-clamp-1 group-hover:text-purple-600 transition-colors">
          {{ offre.titre }}
        </h3>
        <div class="flex items-center text-sm text-gray-600">
          <div class="w-6 h-6 bg-gradient-to-br from-purple-400 to-pink-500 rounded-full flex items-center justify-center text-white text-xs font-bold mr-2">
      ...
      <div v-if="offre.localisation || offre.ville" class="mb-3 flex items-center text-sm text-gray-500">
        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/>
        </svg>
        <span class="truncate">{{ offre.localisation || offre.ville }}</span>
      </div>
      ...
      <div v-if="offre.tags && offre.tags.length" class="mb-3">
        <div class="flex flex-wrap gap-1">
          <span 
            v-for="tag in offre.tags.slice(0, 2)" 
            :key="tag.id"
            class="inline-block bg-purple-50 text-purple-600 text-xs px-2 py-1 rounded-full border border-purple-200"
          >
            {{ tag.nom }}
          </span>
          <span v-if="offre.tags.length > 2" class="text-xs text-gray-400">
            +{{ offre.tags.length - 2 }}
          </span>
        </div>
      </div>
