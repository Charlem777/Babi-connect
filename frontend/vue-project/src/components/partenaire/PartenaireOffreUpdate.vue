    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select v-model="offre.commune" class="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400">
...
onMounted(async () => {
  try {
    ...
    const { data: offreData } = await api.get(`/partenaire/offres/${route.params.id}`)
    console.log("🔍 Données de l'offre chargées :", offreData)
    
    offre.value = {
      ...offreData,
      photos: offreData.photos || [],
      options: offreData.options || [],
      tags: offreData.tags || [],
      expire_le: offreData.expire_le ? offreData.expire_le.split('T')[0] : ''
    }

    console.log("📝 Offre après traitement :", offre.value)

    // Initialiser les tags sélectionnés
    selectedTags.value = offreData.tags ? offreData.tags.map(tag => tag.id) : []
    console.log("🏷️ Tags sélectionnés :", selectedTags.value)
    
    // Initialiser les URLs des photos pour l'affichage
    uploadedPhotoUrls.value = offreData.photos ? offreData.photos.map(photo => `http://localhost:5000${photo}`) : []
    console.log("📸 URLs des photos :", uploadedPhotoUrls.value)
    ...
    message.value = "❌ Erreur lors du chargement de l'offre"
  }
})
...
async function submit() {
  try {
    const payload = {
      ...
      photos: uploadedPhotoUrls.value.map(url => url.replace('http://localhost:5000', ''))
    }

    console.log("Payload envoyé :", payload)

    await api.put(`/partenaire/offres/${route.params.id}`, payload)
    message.value = "✅ Offre modifiée avec succès"
    setTimeout(() => {
      message.value = ""
