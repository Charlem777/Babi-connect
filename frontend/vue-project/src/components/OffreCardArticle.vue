<template>
  <div class="relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group h-96 transform hover:-translate-y-2">
    ...
    <div class="relative h-48 overflow-hidden">
      ...
      />
      <!-- Gradient overlay pour article -->
      <div class="absolute inset-0 bg-gradient-to-t from-green-900/70 via-transparent to-transparent"></div>
      
      <!-- Badge type article -->
      <div class="absolute top-3 left-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
        🛍️ Article
      </div>
      
      <!-- Stock/Disponibilité -->
      <div class="absolute top-3 right-3 bg-emerald-500 text-white px-2 py-1 rounded-lg text-xs font-medium">
        En stock
      </div>
    ...
    <div class="p-4 h-48 flex flex-col">
      ...
      <div class="mb-3">
        ...
        <div class="flex items-center text-sm text-gray-600">
          <div class="w-6 h-6 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center text-white text-xs font-bold mr-2">
      ...
      <!-- Caractéristiques/Tags si disponibles -->
      <div v-if="offre.tags && offre.tags.length" class="mb-3">
        <div class="flex flex-wrap gap-1">
          <span 
            v-for="tag in offre.tags.slice(0, 3)" 
            :key="tag.id"
            class="inline-block bg-green-50 text-green-600 text-xs px-2 py-1 rounded-full border border-green-200"
          >
            {{ tag.nom }}
          </span>
